...
- Email composition via `RFC2822Builder` (RFC 2047 Q-encoding for non-ASCII) sent through Gmail API as multipart/alternative (text/plain + text/html); draft auto-save on compose window close via Gmail Drafts API with optimistic UI update (draft appears immediately, background sync follows); draft editing loads content via Drafts API and sends via `sendDraft`
