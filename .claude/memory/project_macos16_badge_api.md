...
**How to apply:** Use `UNUserNotificationCenter.current().setBadgeCount(_:)` for dock badge. This requires notification permission (`.badge` option in `requestAuthorization`), which the app already requests. The fix is in `AppState.updateDockBadge()` in `AerioApp.swift`.
