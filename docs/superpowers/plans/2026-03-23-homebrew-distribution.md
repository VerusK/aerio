...
**Goal:** Distribute AgMail via `brew install --cask agmail` with automated CI/CD that builds, signs, notarizes, and publishes releases on git tag push.
...
### New files
- `AgMail/Config/OAuth.xcconfig` — xcconfig with placeholder, committed
- `AgMail/Config/OAuth.local.xcconfig.example` — template for local dev
- `AgMail/Config/OAuth.local.xcconfig` — actual local dev credentials (gitignored)
...
### Modified files
- `AgMail/Services/OAuthConfig.swift` — read credentials from Bundle instead of hardcoded
- `AgMail/Services/OAuthManager.swift:80` — use OAuthConfig instead of hardcoded scheme
- `AgMail/Info.plist` — use xcconfig variable for URL scheme
- `AgMail.xcodeproj/project.pbxproj` — add xcconfig reference, update signing for Release
...
**Files:**
- Create: `AgMail/Config/OAuth.xcconfig`
- Create: `AgMail/Config/OAuth.local.xcconfig.example`
- Create: `AgMail/Config/OAuth.local.xcconfig` (gitignored)
- Modify: `AgMail/Services/OAuthConfig.swift`
- Modify: `AgMail/Services/OAuthManager.swift:80`
- Modify: `AgMail/Info.plist`
- Modify: `AgMail.xcodeproj/project.pbxproj`
- Modify: `.gitignore`
...
Save to `AgMail/Config/OAuth.xcconfig`.

- [ ] **Step 2: Create local xcconfig template and actual file**

`AgMail/Config/OAuth.local.xcconfig.example`:
```
...
`AgMail/Config/OAuth.local.xcconfig`:
```
...
2. Set `OAuth.xcconfig` as the configuration file for both Debug and Release configurations of the AgMail target
...
Merge into the existing `AgMail/Info.plist` — keep all existing keys and add the OAuth ones. The result should be:
...
<dict>
	...
	<array>
		<dict>
			...
			<string>com.agmail.oauth</string>
...
```bash
xcodebuild -project AgMail.xcodeproj -scheme AgMail -configuration Release build OAUTH_CLIENT_ID=test-client-id
```
...
```bash
git add AgMail/Config/OAuth.xcconfig AgMail/Config/OAuth.local.xcconfig.example \
  AgMail/Services/OAuthConfig.swift AgMail/Services/OAuthManager.swift \
  AgMail/Info.plist AgMail.xcodeproj/project.pbxproj .gitignore
...
- `GOOGLE_CLIENT_ID`
- `TAP_GITHUB_TOKEN` — PAT with repo access to `VerusK/homebrew-agmail`
  ...
  release:
    ...
    steps:
      ...
      - name: Archive
        run: |
          xcodebuild archive \
            ...
            -archivePath $RUNNER_TEMP/AgMail.xcarchive \
      ...
      - name: Notarize DMG
        ...
        run: |
          DMG_PATH="$RUNNER_TEMP/AgMail-${{ steps.version.outputs.version }}.dmg"
      ...
      - name: Create GitHub Release
        ...
        run: |
          ...
          gh release create "$GITHUB_REF_NAME" "$DMG_PATH" \
            --title "AgMail ${{ steps.version.outputs.version }}" \
            --generate-notes
      ...
      - name: Update Homebrew tap
        ...
        run: |
          ...
          git clone https://x-access-token:${TAP_TOKEN}@github.com/VerusK/homebrew-agmail.git $RUNNER_TEMP/tap
          cd $RUNNER_TEMP/tap

          # Update version and sha in Cask formula (sed to avoid heredoc indentation issues)
          sed -i '' "s/version \".*\"/version \"${VERSION}\"/" Casks/agmail.rb
          sed -i '' "s/sha256 \".*\"/sha256 \"${SHA}\"/" Casks/agmail.rb

          git config user.name "github-actions[bot]"
          git config user.email "github-actions[bot]@users.noreply.github.com"
          git add Casks/agmail.rb
          git commit -m "Update AgMail to ${VERSION}"
          git push
...
Create the `VerusK/homebrew-agmail` repository with initial Cask formula.

**Files (in separate repo):**
- Create: `VerusK/homebrew-agmail` repo on GitHub
- Create: `Casks/agmail.rb`
...
```bash
gh repo create VerusK/homebrew-agmail --public --description "Homebrew tap for AgMail"
```

- [ ] **Step 2: Clone and add initial Cask formula**

```bash
git clone git@github.com:VerusK/homebrew-agmail.git /tmp/homebrew-agmail
cd /tmp/homebrew-agmail
mkdir -p Casks
```
...
- Name: `agmail-tap-update`
- Repository access: Only `VerusK/homebrew-agmail`
- Permissions: Contents (Read and Write)

Save the token as `TAP_GITHUB_TOKEN` secret in the `VerusK/agapp` repository.
...
In `VerusK/agapp` repo Settings > Secrets and variables > Actions, add:
...
Expected: AgMail installs and launches. OAuth login works.

- [ ] **Step 5: Verify on clean machine (optional)**

Test on a machine that has never had AgMail installed to confirm Gatekeeper doesn't block it.
